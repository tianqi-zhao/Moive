...
<script type="text/javascript">
   ...
   function movieSubmit() {
       ...
       if(ids!="") {
           $.post("<%=path%>/customer/register/movieSubmit", {'ids': ids}, function (data) {
               if(data=="ok") {
                   alert("提交成功");
                   $('#myModal').modal('hide');
...
<script type="text/javascript">
    ...
    $("#regName").blur(function () {
        ...
        $.ajax({url:surl + "<%=path%>/customer/check/"+username+"/1?r=" + Math.random(),
...
//        $(".mail").hide();
        ...
        //判断邮箱是否被占用
        var surl = "";
        $.ajax({url:surl + "<%=path%>/customer/check/"+escape($("#email").val())+"/3?r=" + Math.random(),
            success : function(data) {
                if (data.data) {
                } else {
                    $(".emailerror").show();
...
//                    style = 'position: relative ;left: 290px;'
    ...
    var REGISTER={
        ...
        beforeSubmit:function() {
            ...
            //检查用户和邮箱是否已经被占用
            $.ajax({
                url : REGISTER.param.surl + "<%=path%>/customer/checkboth/"+username+"/"+escape($("#email").val())+"/4?=" + Math.random(),
                success : function(data) {
                    if (data.data) {
                        REGISTER.doSubmit();
                    } else {
        ...
        doSubmit:function() {
            $.post("<%=path%>/customer/register",$("#regForm_mod").serialize(), function(data){
                if(data.status == 200){
...
<script type="text/javascript">
    ...
    var arr = new Array();
    arr[0] = "<%=basePath%>assets/img/loginimg/1.jpg";
    arr[1] = "<%=basePath%>assets/img/loginimg/2.jpg";
    arr[2] = "<%=basePath%>assets/img/loginimg/3.jpg";
