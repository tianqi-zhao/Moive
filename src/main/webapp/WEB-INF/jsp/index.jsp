...
<nav class="navbar navbar-default" role="navigation" style="background-color: black;margin-bottom: 0%">
    <a class="navbar-brand" href="" style="color: white">电影</a>
    ...
    <a class="navbar-brand" href="<%=path%>/index" style="color: white">选电影</a>
    <!-- 判断用户是否登录-->
            <c:if test="${sessionScope.user == null}">
            <a  class="dream" href="javascript:window.location.href='<%=path%>/page/register'" id="register" style="float: right;color: white;font-size: 13pt;margin-top: 10px;margin-right: 10px"><span style="color: white" class="glyphicon glyphicon-user"></span> 注册</a>
            <a  class="dream" href="javascript:window.location.href='<%=path%>/page/login'" style="float: right;color: white;font-size: 13pt;margin-top: 10px;margin-right: 10px"><span style="color: white" class="glyphicon glyphicon-log-in"></span> 登录</a>
            </c:if>
            <c:if test="${sessionScope.user != null}">

                <a class="dream" id="logout" href="javascript:window.location.href='<%=path%>/page/logout'" style="float: right;color: white;font-size: 13pt;margin-top: 10px;margin-right: 10px"><span style="color: white" class="glyphicon glyphicon-log-in"></span>  退出</a>
                <a class="dream" onclick='javascript:$.post("<%=path%>/page/profile",{"id":"${sessionScope.user.userid}"}, function (data) {
            if (data=="success") {
                location.href = "<%=path%>/page/profile"
            } else {
            }
                        ...
                        <div id="list" class="list">
                            ...
                            <c:if test="${sessionScope.movie != null}">
                                <c:forEach var="item"   items="${sessionScope.movie}">
                                    <a class="item"   target="_blank" name="imgitem" id="${item.movieid}" onclick='javascript:$.post("<%=path%>/Customer/Description",{id:$(this).attr("id")}, function (data) {
                                    if (data == "success") {
                                        location.href = "<%=path%>/MovieDescription"
                                    } else {
                                    }
                                    })'>
                                        <div class="cover-wp">
                                    ...
                                    <c:forEach var="item"   items="${sessionScope.TopDefaultMovie}" varStatus="i">
                                        <c:if test="${i.count==1&&i.count<6}">
                                            <div class="item active">
                                                <img src="${item.backpost}" alt="${item.movieid}" onclick='javascript:$.post("<%=path%>/Customer/Description",{id:$(this).attr("alt")}, function (data) {
                                                if (data=="success") {
                                                    location.href = "<%=path%>/MovieDescription"
                                                } else {
                                                }
...
<script>
    $(document).on("click",'#loadmore',function() {
        $.post("<%=path%>/loadingmore",{molimit:$("#list").children("a").length,type:$("label[class='activate']").attr("value"),sort: $("input[name='sort']:checked").val()},
            function (data) {
            if (data.status == 200) {
...
<script type="text/tmpl" id="subject-tmpl">
        ...
        <a class="item"  name="imgitem" target="_blank" id="{id}" onclick='javascript:$.post("<%=path%>/Customer/Description",{id:$(this).attr("id")}, function (data) {
            if (data=="success") {
                location.href = "<%=path%>/MovieDescription"
            } else {
            }
...
<script>
    ...
    $("#inp-query").bind("keyup",function () {
        ...
        $.post("<%=path%>/search",{"search_text":searchText},function (data) {
            if (data.status == 200) {
                if(data.data.length!=0) {
                    $.each(data.data, function (i, item) {
...
<script type="text/tmpl"  id="movie-tmpl">
 <li id="searchResult">
   <div>
      <a value="{id}" style="text-decoration:none" onclick='javascript:$.post("<%=path%>/Customer/Description",{id:$(this).attr("value")}, function (data) {
            if (data=="success") {
                location.href = "<%=path%>/MovieDescription"
            } else {
            }
