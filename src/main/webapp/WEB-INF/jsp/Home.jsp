...
<nav class="navbar navbar-default" role="navigation" style="background-color: #222;margin-bottom: 0%">
    ...
    <c:if test="${sessionScope.user == null}">
        <a  class="dream" href="javascript:window.location.href='<%=path%>/page/register'" id="register" style=" text-decoration:none;float: right;color: white;font-size: 13pt;margin-top: 12px;margin-right: 10px"><span style="color: white" class="glyphicon glyphicon-user"></span> 注册</a>
        <a  class="dream" href="javascript:window.location.href='<%=path%>/page/login'" style=" text-decoration:none;float: right;color: white;font-size: 13pt;margin-top: 12px;margin-right: 10px"><span style="color: white" class="glyphicon glyphicon-log-in"></span> 登录</a>
    </c:if>
    <c:if test="${sessionScope.user != null}">

        <a class="dream" id="logout" href="javascript:window.location.href='<%=path%>/page/logout'" style=" text-decoration:none;float: right;color: white;font-size: 13pt;margin-top: 12px;margin-right: 10px"><span style="color: white" class="glyphicon glyphicon-log-in"></span>  退出</a>
...
<div class="fm-discovery" id="wholediv" style="background-image: url('${sessionScope.TopDefaultMovie[0].backpost}')">
    ...
    <div class="x-kankan">
        ...
        <div id="x-kankan-detail" class="x-kankan-detail">
            <p class="x-kankan-title">
                <a name="movienametag" onclick='javascript:$.post("<%=path%>/Customer/Description",{id:$(this).attr("value")}, function (data) {
            if (data=="success") {
        ...
        <a class="fm-green" value="${sessionScope.TopDefaultMovie[0].movieid}" onclick='javascript:$.post("<%=path%>/Customer/Description",{id:$(this).attr("value")}, function (data) {
            if (data=="success") {
                location.href = "<%=path%>/MovieDescription"
            } else {
            }
    ...
    <div class="x-usermovie-controls x-kankan-buttons">
        ...
        <div class="btn-group fm-discovery-actions">
            <!-- 搜索影片资源跳转详情页-->
            <a name="moviedesc" data-placement="top" onclick='javascript:$.post("<%=path%>/Customer/Description",{id:$(this).attr("value")}, function (data) {
            if (data=="success") {
                location.href = "<%=path%>/MovieDescription"
            } else {
            }
