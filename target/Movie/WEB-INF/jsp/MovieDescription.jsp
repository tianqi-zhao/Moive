...
<nav class="navbar navbar-default" role="navigation" style="background-color: black;margin-bottom: 0%">
    ...
    <c:if test="${sessionScope.user == null}">
        <a class="dream" href="javascript:window.location.href='<%=path%>/page/register'" id="register"
        ...
        <a class="dream" href="javascript:window.location.href='<%=path%>/page/login'"
           style="float: right;color: white;font-size: 13pt;margin-top: 10px;margin-right: 10px"><span
                style="color: white" class="glyphicon glyphicon-log-in"></span> 登录</a>
    </c:if>
    <c:if test="${sessionScope.user != null}">

        <a class="dream" id="logout" href="javascript:window.location.href='<%=path%>/page/logout'"
           style="float: right;color: white;font-size: 13pt;margin-top: 10px;margin-right: 10px"><span
                    ...
                    <div class="col-md-5 col-sm-12 film-stats" style="">
                        ...
                        <c:if test="${sessionScope.user != null&&sessionScope.userstar==null}">
                            <button id="submitevalutionstar" class="btn btn-default btn-md"
                                    onclick='$.post("<%=path%>/getstar",{userid:${sessionScope.user.userid},movieid:${sessionScope.moviedescription.movieid},time:getNowFormatDate(),star:$("#Evaluation").val()},function (data) {
...
<script>
    $('#reviewsId').click(function (event) {
        ...
        $.post("<%=path%>/getSimiMovies", {"id": "${sessionScope.moviedescription.movieid}"},function (data) {
            if (data.status == 200) {
...
<script>
    function  likedclick() {
        ...
        if(color=="rgb(230, 230, 230)")
            ...
            boollike=0;
        $.post("<%=path%>/likedmovie", {"movieid": "${sessionScope.moviedescription.movieid}","boollike":boollike,"userid":"${sessionScope.user.userid}"},function (data) {
            if(data=="success") {
                if (boollike == 1)
                    alert("收藏成功");
                else
...
<script type="text/tmpl" id="recommodmovies">
    <tr>
    <td>
    <a value="{id}" onclick='javascript:$.post("<%=path%>/Customer/Description",{id:$(this).attr("value")}, function (data) {
            if (data=="success") {
